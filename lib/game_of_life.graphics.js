(function (GoL, _) {
  ...
  GoL.Graphics = function (canvas, grid) {
    var context         = canvas.getContext('2d'),
        ...
        countBoxHeight  = 16,
        countBoxTextPos = [canvasWidth - 4, canvasHeight - 4];
    ...
    function drawGrid() {
      ...
      for (x = cellSize + 0.5; x < canvasWidth; x += cellSize) {
        context.moveTo(x, 0);
        context.lineTo(x, canvasHeight);
      }

      for (y = cellSize + 0.5; y < canvasHeight; y += cellSize) {
        context.moveTo(0, y);
    ...
    function drawGenerationCount(count) {
      var countBoxWidth = (count.toString()).length * 10 + 4;
